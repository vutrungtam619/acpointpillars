...
class InvertedResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, expand_ratio=6, kernel_type='3x3'):
        super().__init__()
        ...
        if kernel_type not in kernel_map:
            raise ValueError("kernel_type must be '3x3', '3x1', or '1x3'")
        kernel_size, padding = kernel_map[kernel_type]
...
class Backbone(nn.Module):
    def __init__(self, in_channels, out_channels, layer_nums, layer_strides, expand_ratio, kernel_type):
        super().__init__()
        ...
        for i in range(len(layer_strides)):
            blocks = []
            blocks.append(InvertedResidualBlock(in_channels, out_channels[i], stride=layer_strides[i], expand_ratio=expand_ratio, kernel_type=kernel_type))
            for _ in range(layer_nums[i]):
                blocks.append(InvertedResidualBlock(out_channels[i], out_channels[i], stride=1, expand_ratio=expand_ratio, kernel_type=kernel_type))
    ...
    def forward(self, x):
        ...
        Returns:
            features [list of torch.tensor]: (B, C, H/2, W/2), (B, 2C, H/4, W/4), (B, 4C, H/8, W/8)
        '''
        features = []
...
class BottleNeck(nn.Module):
    def __init__(self, in_channels, out_channels, upsample_strides):
        ...
        for i in range(len(in_channels)):
            decoder_block = []
            reduced_channel = in_channels[i] // 2
            decoder_block.append(nn.Conv2d(in_channels[i], reduced_channel, kernel_size=1, stride=1, bias=False))
            decoder_block.append(nn.BatchNorm2d(reduced_channel))
            decoder_block.append(nn.ReLU(inplace=True))
            decoder_block.append(nn.ConvTranspose2d(reduced_channel, out_channels[i], upsample_strides[i], stride=upsample_strides[i], bias=False))
    ...
    def forward(self, x):
        '''
        Args: 
            x [list of torch.tensor float32]: (B, 3C, H/2, W/2), (B, 6C, H/4, W/4), (B, 12C, H/8, W/8)    
        Returns:
            feature_map [torch.tensor float32, (B, 6C, H/2, W/2)]
        '''
        feature_map = []
        for i in range(len(self.decoder_blocks)):
            deconv_blocks = self.decoder_blocks[i](x[i]) 
...
class PointPillars(nn.Module):
    def __init__(
        ...
        self.se1 = SEBlock(channels=192, reduction=4)
        self.se2 = SEBlock(channels=384, reduction=8)
        self.se3 = SEBlock(channels=768, reduction=16)
    ...
    def forward(self, batched_pts, mode, batched_gt_bboxes, batched_gt_labels):
        ...
        block3x3 = self.backbone3x3(pillar_features) # [(B, C, H/2, W/2), (B, 2C, H/4, W/4), (B, 4C, H/8, W/8)]
        block3x1 = self.backbone3x1(pillar_features) # [(B, C, H/2, W/2), (B, 2C, H/4, W/4), (B, 4C, H/8, W/8)]
        block1x3 = self.backbone1x3(pillar_features) # [(B, C, H/2, W/2), (B, 2C, H/4, W/4), (B, 4C, H/8, W/8)]
        
        stage1 = torch.cat([block3x3[0], block3x1[0], block1x3[0]], dim=1) # (B, 3C, H/2, W/2)
        stage2 = torch.cat([block3x3[1], block3x1[1], block1x3[1]], dim=1) # (B, 6C, H/4, W/4)
        stage3 = torch.cat([block3x3[2], block3x1[2], block1x3[2]], dim=1) # (B, 12C, H/8, W/8)
