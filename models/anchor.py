...
class Anchors():
    ...
    def get_anchors(self, feature_map_size, anchor_range, anchor_size, rotations):
        ...
        meshgrids = torch.meshgrid(x_centers, y_centers, z_centers, rotations, 'ij')
...
def anchor_target(batched_anchors, batched_gt_bboxes, batched_gt_labels, assigners, nclasses):
    ...
    for i in range(batch_size):
        ...
        for j, assigner in enumerate(assigners):
            ...
            if gt_bboxes.numel() == 0:
                ...
                assigned_gt_labels = torch.full((cur_anchors.size(0),), nclasses, dtype=torch.long, device=device)
                assigned_gt_labels_weights = torch.ones(cur_anchors.size(0), device=device)
                assigned_gt_reg = torch.zeros_like(cur_anchors, device=device)
                assigned_gt_reg_weights = torch.zeros(cur_anchors.size(0), device=device)
                assigned_gt_dir = torch.zeros(cur_anchors.size(0), dtype=torch.long, device=device)
                assigned_gt_dir_weights = torch.zeros(cur_anchors.size(0), device=device)
            else:
                # --- logic anchor_target bình thường ---
                ...
                assigned_gt_labels = torch.full((cur_anchors.size(0),), nclasses, dtype=torch.long, device=cur_anchors.device)
    ...
    rt_dict = dict(
        ...
        batched_dir_labels=torch.stack(batched_dir_labels, 0),
        batched_dir_labels_weights=torch.stack(batched_dir_labels_weights, 0)
    )
    return rt_dict
